...
class QuitGroupTool(Tool):
    """Quit RongCloud group tool class
    
    Used to quit RongCloud group, need to provide user ID and group ID.
    ...
    def _invoke(self, tool_parameters: Dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = rongcloud_api._make_request("POST", "group/quit.json", data)
            yield self.create_json_message(result.json())
        except Exception as e:
            raise Exception(f"Quit group failed: {str(e)}")
