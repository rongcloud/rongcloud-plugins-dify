...
class GetGroupMessagesTool(Tool):
    """Get group messages from RongCloud tool class
    
    Used to get group messages from RongCloud, need to provide message ID.
    The tool will return group messages, which can be used for subsequent RongCloud API calls.
    """
    ...
    def _invoke(self, tool_parameters: Dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            result = rongcloud_api._make_request("POST", "v3/message/group/query.json", data, is_json=True)
            yield self.create_json_message(result.json())
        except Exception as e:
            raise Exception(f"Get group messages failed: {str(e)}")
    ...
    def get_request_data(self, tool_parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        request_data['startTime'] = current_time + 1000 * 60 * 60
        # The last day
